...
- **THEN** o arquivo SQLite no ZIP reflete um estado consistente (sem truncamento WAL parcial)
...
#### Scenario: Schema mais antigo
- **WHEN** o ZIP tem `schemaVersion` < schema do app
- **THEN** o restore é permitido e as migrations Drift aplicam-se na reabertura
...
### Requirement: Aplicar prefs do backup
Se o ZIP contiver `prefs.json` válido, o restore SHALL reaplicar essas preferências após substituir o banco.

#### Scenario: Troca de PC
- **WHEN** um backup completo é restaurado em máquina nova
- **THEN** broker MQTT / laser / station_id ficam alinhados ao backup (sujeito às chaves incluídas em `prefs.json`)
